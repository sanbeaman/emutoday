<div id="connection-bar" data-equalizer>
  <div id="all-connections" data-equalizer-watch>
    <div id="white-bar">
      <div id="tier1-nav">
        <div class="row">
          <div class="large-9 large-push-3 medium-12 small-12 columns">
            <div class="row">
              <div class="large-5 medium-9 small-12 columns">
                <h3 class="magazine-main-title"><a href="">Eastern <span class="magazine-descriptor">The Magazine of Eastern Michigan University</span><span class="magazine-descriptor-small">Magazine</span></a></h3>
              </div>
              <div class="large-7 medium-3 small-12 columns">
                <ul>
                  <li class="search-area"><a class="search-glass" href="">Search</a></li>
                  <li class="menu-area"><a class="right-off-canvas-toggle menu-icon" href="">Menu</a></li>
                </ul>
              </div>
            </div>
          </div><!-- large-9 -->
        <div class="large-3 large-pull-9 hide-for-medium hide-for-small columns">
            <div id="logo-box-magazine" data-equalizer-watch>
                 <a href=""><img class="magazine-logo" alt="Eastern Michigan University" src="/themes/default/assets/imgs/magazine/eatern-mag-logo.png"></a>

            </div><!-- logo-box-magazine -->
          </div><!-- large-3 -->
        </div><!-- row -->
      </div><!--tier1-nav -->
    </div><!-- white-bar -->
    <div id="transparent-bar">
      <div id="tier2-nav" class="row">
        <div class="large-10 large-push-2 medium-12 small-12 columns">
          <div class="row">
            <div class="large-12 medium-12 small-12 columns">
              <!-- '/admin/php/top_nav.php'); -->
              <ul>
                <li><a href="#">Current Issue</a></li>
                <li><a href="#">Past Issues</a></li>
                <li><a href="#">Alumni</a></li>
                <li><a href="#">EMU Today</a></li>
              </ul>
            </div>
          </div>
        </div>
        <div class="large-2 large-pull-10 hide-for-medium hide-for-small columns">&nbsp;</div>
      </div> <!-- tier2-nav -->
    </div><!-- transparent-bar -->
    <div id="secondary-bar">
      <div id="tier3-nav" class="row">
        <div class="large-10 large-push-2 medium-12 hide-for-small columns">
          <!-- div row removed addedback-->
          <div class="row">
            <div class="large-12 medium-12 small-12 columns">
              <ul>
                <!-- '/admin/php/secondary_nav.php'); -->
                <li><a href="#">Subscribe to Eastern Magazine</a></li>
                <li><a href="#">Submit a Story idea</a></li>
              </ul>
            </div>
          </div>
          <!-- div row removed - addedback -->
        </div>
        <div class="large-2 large-pull-10 hide-for-medium hide-for-small columns">&nbsp;</div>
      </div>
    </div><!-- secondary-bar -->
  </div> <!-- all-connections -->
</div> <!-- connection-bar -->
